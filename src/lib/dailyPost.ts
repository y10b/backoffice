...
async function pickFromSignals(recent: Set<string>): Promise<Choice | null> {
  const signals = await topicSignals({ limit: 10 });
  const hit = signals.find((s) => !recent.has(bare(s.query)));
  return hit ? { source: "signal", seed: "", keyword: hit.query } : null;
}

async function pickFromPool(recent: Set<string>): Promise<Choice | null> {
  const rows = (await listKeywordPool({ days: POOL_DAYS, sort: "value", limit: 500 }))
    .filter((r) => (r.bid ?? 0) >= MIN_BID && !recent.has(bare(r.keyword)))
    .sort(byValue);
  const r = rows[0];
  return r
    ? {
...
async function pickFromSeed(seed: string, recent: Set<string>): Promise<{ pick: Choice; related: Keyword[] }> {
  ...
  const picked =
    strict.find((k) => !recent.has(bare(k.keyword))) ??
    loose.find((k) => !recent.has(bare(k.keyword))) ??
    all[0] ??
    research.keywords[0];
  return {
    pick: {
...
export async function writeDailyPost(
  ...
  if (!explicitSeed) {
    ...
    if (!pick) {
      try {
        pick = await pickFromSignals(recent);
      } catch (e) {
    ...
    if (!pick) {
      try {
        pick = await pickFromPool(recent);
      } catch (e) {
  ...
  if (!pick) {
    ...
    try {
      const r = await pickFromSeed(seed, recent);
      pick = r.pick;
      related = r.related;
    } catch (e) {
