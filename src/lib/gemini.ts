...
export function buildPrompt(
  ...
  return [
    "당신은 검색엔진과 AI 답변엔진 양쪽에 인용되는 글을 쓰는 한국어 작가입니다.",
    "아래 조건으로 티스토리(주력)와 네이버 블로그에 함께 발행할 글을 작성하세요.",
    "",
    "읽는 쪽이 넷입니다. 각각 원하는 게 다릅니다.",
    "- 구글·네이버 크롤러: 검색 의도를 얼마나 빨리, 정확히 충족하는가 (키워드 밀도가 아님)",
    "- AI 답변엔진(구글 AI Overviews·빙 Copilot): 답이 되는 **문장을 뽑아 쓴다**",
    "- 생성 AI(ChatGPT·Perplexity): 그 숫자의 **원출처**를 인용한다",
    "- 네이버 AI 브리핑: **문단 단위**로 잘라 출처 칩을 단다",
    "",
    "그래서 글 전체가 좋은 것만으로는 부족합니다. **문단 하나하나가 따로 떼어져도 말이 되어야** 합니다.",
    "",
    `## 메인 키워드\n${o.mainKeyword}`,
    `## 서브 키워드\n${o.subKeyword}`,
    outline ? `## 반드시 다룰 내용\n${outline}` : "",
    ...
    "",
    "## 제목 규칙",
    "- 제목은 `본제목 - 부제` 형태로 쓸 것. 본제목에 메인 키워드, 부제에 서브 키워드를 담는다.",
    "- 메인·서브 키워드는 **둘 다 원형 그대로** 넣을 것 (변형·조사 삽입으로 키워드가 끊기지 않게).",
    "- 부제는 검색량이 확인된 키워드다. 억지로 늘이지 말고 그 키워드가 답하는 질문을 짧게 붙일 것.",
    "- 서브 키워드가 비어 있으면 부제 없이 본제목만 쓸 것. 없는 걸 지어내지 말 것.",
    "- 메인 키워드를 앞쪽에 배치할 것.",
    "- 공백 포함 32자 내외. 낚시성 과장 표현 금지.",
    "",
    "## 인용되는 글쓰기 (가장 중요)",
    "- **첫 문단에서 직답**: 독자가 검색한 질문의 답을 40자 내외 한 문장으로 맨 앞에. 배경 설명·인사말로 시작하지 말 것.",
    "  예: `결론부터: 2026년 청년도약계좌 가입은 만 19~34세, 연소득 7,500만원 이하면 가능합니다.`",
    "- **모든 문단이 홀로 서게**: 문단을 통째로 떼어내 다른 곳에 붙여도 뜻이 통해야 합니다.",
    "  - 금지: `위에서 말한 그 금액`, `앞서 설명한 조건`, `이것은`, `해당 제도는` 처럼 앞 문단에 기대는 표현",
    "  - 대신: 문단마다 주어를 다시 밝힐 것. `청년도약계좌는…`",
    "- **숫자에는 기준을 붙일 것**: 기준 없는 숫자는 답변엔진이 신뢰 판정에서 감점합니다.",
    "  - 나쁨: `지원금은 월 최대 2.4만원입니다`",
    "  - 좋음: `지원금은 월 최대 2.4만원입니다 (2026년 기준, 연소득 2,400만원 이하 구간)`",
    "- **출처를 문장 안에 밝힐 것**: 어디서 온 숫자인지 적으면 인용 가치가 올라갑니다. `보건복지부 공고 기준`, `국세청 발표 기준` 처럼.",
    "- 상세한 근거·조건·예외는 직답 뒤에 이어서 설명할 것.",
    "",
    "## 헤딩 계층",
    ...
    "- 조건·금액·기간처럼 항목이 정해진 정보는 **산문이 아니라 라벨-값 구조**(표 또는 `- 항목: 값` 목록)로 쓸 것.",
    "  네이버 AI 브리핑은 줄글 덩어리보다 라벨-값 구조를 인용합니다.",
    ...
    "- 수치를 쓸 때는 **어디서 온 것인지** 함께 적을 것. 익명의 숫자보다 출처가 밝혀진 숫자가 인용됩니다.",
    ...
    "- 예측·권유형 질문은 넣지 말 것. 데이터로 확정되는 것만 (특히 의료·법률·금융).",
    "- 같은 내용을 `faq` 필드에도 질문/답변 쌍으로 넣을 것(구조화 데이터용).",
    "- **본문의 FAQ 와 `faq` 필드는 글자까지 같아야 합니다.** 구조화 데이터가 화면에 안 보이는 말을 하면 스팸으로 판정됩니다.",
    "  본문 FAQ 답변을 마크다운 없이 평문으로 쓰고, 그 문장을 `faq` 에 그대로 복사할 것.",
    "",
    "## 시각 자료 규칙 (`visuals`)",
